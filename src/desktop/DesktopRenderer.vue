<template>
  <DesktopShell
    ...
    :grade-label="`${demoData.student.grade} · ${demoData.student.className}`"
  ...
  >
    <div class="pc-page" :data-font-size="fontSize">
      ...
      <section v-if="screenKey === 'previewLesson'" class="pc-preview-lesson-page pc-layout-2">
        <article class="pc-card pc-main-card">
          ...
          <template v-if="previewGenerated">
            ...
            <div v-if="previewFormat === 'html'" class="pc-preview-slide"><span>{{ previewSubject }} · {{ previewTopic }} · 第 {{ previewSlideIndex + 1 }} / {{ previewSlides.length }} 页</span><h3>{{ previewSlides[previewSlideIndex].title }}</h3><p>{{ previewSlides[previewSlideIndex].summary }}</p><div class="pc-preview-interactions"><button v-for="item in previewSlides[previewSlideIndex].points" :key="item" :class="{ active: selectedPreviewPoint === item }" @click="selectedPreviewPoint = item">{{ item }}</button></div><div class="pc-tint-note">{{ previewSlides[previewSlideIndex].detail }}</div></div>
            <pre v-else class="pc-text-ppt-preview">{{ previewArtifactText }}</pre>
            <div class="pc-preview-actions"><button class="pc-secondary" :disabled="previewSlideIndex === 0" @click="previewSlideIndex -= 1">上一页</button><button class="pc-primary" :disabled="previewSlideIndex === previewSlides.length - 1" @click="previewSlideIndex += 1">下一页 <ArrowRight :size="17" /></button><button class="pc-primary" @click="runFeaturePrimaryAction">根据当前课题出题练习 <ArrowRight :size="17" /></button></div>
      ...
      <template v-else-if="screenKey === 'wrongBook' || screenKey === 'wrongBookDetail'"><section class="pc-layout-2"><article class="pc-card pc-main-card"><div class="pc-card-title"><div><span class="pc-eyebrow">错题整理</span><h2>{{ currentWrongTopic.title }}</h2></div><span class="pc-pill">{{ wrongBookTopics.length }} 个专题</span></div><p class="pc-lead">{{ currentWrongTopic.summary }}</p><div class="pc-topic-list"><button v-for="topic in wrongBookTopics" :key="topic.key" :class="['pc-topic', { active: topic.key === selectedWrongBookTopicKey }]" @click="openWrongBookDetail(topic.key)"><span><strong>{{ topic.title }}</strong><small>{{ topic.meta }} · {{ topic.commonMistake }}</small></span><b>{{ topic.count }} 题</b></button></div><div v-if="screenKey === 'wrongBookDetail'" class="pc-detail-list"><article v-for="question in currentWrongTopic.questions" :key="question.id" class="pc-detail-row"><div><strong>{{ question.prompt }}</strong><small>{{ question.reason }}</small></div><span class="pc-answer-compare"><b class="pc-answer-wrong">作答 {{ question.studentAnswer }}</b><b class="pc-answer-correct">正确 {{ question.correctAnswer }}</b></span></article></div><button class="pc-primary" @click="startPracticeFromSource(wrongBookSourceMap[selectedWrongBookTopicKey])">进入这个专题练习 <ArrowRight :size="18" /></button></article><aside class="pc-side-stack"><article class="pc-card"><h3>本次练习目标</h3><strong class="pc-card-value">{{ currentWrongTopic.practiceGoal }}</strong><p>{{ currentWrongTopic.commonMistake }}</p></article><article class="pc-card pc-tint-card"><h3>家长怎么陪</h3><p>先让孩子说出每一步怎么想，再做同类确认题，不要直接告诉答案。</p></article></aside></section></template>
      ...
      <template v-else-if="isReviewScreen"><section v-if="screenKey === 'afterClassReview'" class="pc-layout-2"><article class="pc-card pc-main-card"><span class="pc-eyebrow">课后复习 · 准备</span><h2>先选好今天要复习的内容</h2><div class="pc-form-grid pc-review-select-grid"><label class="pc-field"><span>选择学科</span><select v-model="afterClassSubject" @change="changeAfterClassSubject"><option v-for="subject in afterClassSubjectOptions" :key="subject" :value="subject">{{ subject }}</option></select></label><label class="pc-field"><span>选择课题</span><select v-model="afterClassTopic"><option v-for="topic in afterClassTopicOptions" :key="topic" :value="topic">{{ topic }}</option></select></label></div><button class="pc-primary" @click="startAfterClassReview">生成并开始复习小测 <ArrowRight :size="18" /></button></article><aside class="pc-card"><h3>会这样进行</h3><div class="pc-step-list"><div v-for="(step,index) in reviewSteps" :key="step"><b>{{ index + 1 }}</b><span>{{ step }}</span></div></div></aside></section><section v-else-if="screenKey === 'afterClassReviewPaper'" class="pc-layout-2"><article class="pc-card pc-main-card"><div class="pc-card-title"><div><span class="pc-eyebrow">课后复习 · 答题</span><h2>{{ afterClassSubject }} · {{ afterClassTopic }} · 课后复习小测</h2></div><span class="pc-pill">{{ afterClassReviewExam.questions.length }} 道题</span></div><div class="pc-review-list"><article v-for="question in afterClassReviewExam.questions" :key="question.id" class="pc-review-question"><div class="pc-card-title"><strong>{{ question.id }}. {{ question.prompt }}</strong><small>{{ question.type }}</small></div><div class="pc-option-grid"><button v-for="answer in question.options" :key="answer" :class="['pc-option', { active: reviewSelections[question.id] === answer }]" @click="reviewSelections[question.id] = answer">{{ answer }}</button></div></article></div><div class="pc-test-tools"><button class="pc-secondary" @click="toggleSpeech(afterClassReviewExam.title + '。' + afterClassReviewExam.questions.map((question) => question.prompt).join('。'))"><VolumeX v-if="isSpeaking" :size="16" /><Volume2 v-else :size="16" />{{ isSpeaking ? "停止朗读" : "朗读试题" }}</button><button class="pc-secondary" @click="downloadReview"><Download :size="16" />下载试题</button></div><button class="pc-primary" @click="handleAction('SUBMIT_AFTER_CLASS_REVIEW')">提交小测 <ArrowRight :size="18" /></button></article><aside class="pc-card"><h3>答题规则</h3><p>{{ afterClassReviewExam.rule }}</p><div class="pc-tint-note">提交后统一分析正确率、错题原因和薄弱知识点。</div></aside></section><section v-else-if="screenKey === 'afterClassReviewResult'" class="pc-layout-2"><article class="pc-card pc-main-card"><span class="pc-eyebrow">课后复习 · 结果</span><h2>这次不是只看对错，而是找到卡点</h2><div class="pc-metric-row"><div><strong>{{ reviewCorrectCount }} / {{ afterClassReviewExam.questions.length }}</strong><span>本次正确</span></div><div><strong>{{ afterClassReviewExam.weakPoints.length }}</strong><span>主要薄弱点</span></div></div><div class="pc-detail-list"><div v-for="question in afterClassReviewExam.questions" :key="question.id" class="pc-detail-row"><div><strong>{{ question.prompt }}</strong><small>{{ question.knowledgePoint }}</small></div><span>作答 {{ question.studentAnswer }}<br />正确 {{ question.correctAnswer }}</span></div></div></article><aside class="pc-card pc-action-panel"><h3>系统找到的不懂点</h3><div v-for="point in afterClassReviewExam.weakPoints" :key="point.title" class="pc-tint-note"><strong>{{ point.title }}</strong><p>{{ point.evidence }}</p></div><button class="pc-primary" @click="handleAction('OPEN_AFTER_CLASS_KNOWLEDGE')">看知识点讲解</button></aside></section><section v-else class="pc-layout-2"><article class="pc-card pc-main-card"><span class="pc-eyebrow">课后复习 · 知识点</span><h2>{{ afterClassReviewExam.knowledgeExplain.title }}</h2><p class="pc-lead">{{ afterClassReviewExam.knowledgeExplain.summary }}</p><div class="pc-tint-note">{{ afterClassReviewExam.knowledgeExplain.coreRule }}</div><div class="pc-step-list"><div v-for="(step,index) in afterClassReviewExam.knowledgeExplain.steps" :key="step"><b>{{ index + 1 }}</b><span>{{ step }}</span></div></div></article><aside class="pc-card"><h3>家长怎么协助</h3><p>{{ afterClassReviewExam.knowledgeExplain.familyGuide }}</p><div class="pc-chip-list"><span v-for="mistake in afterClassReviewExam.knowledgeExplain.commonMistakes" :key="mistake">{{ mistake }}</span></div><button class="pc-primary" @click="handleAction('VIEW_PROGRESS')">看学习报告</button></aside></section></template>

      <template v-else-if="isPracticeScreen"><section class="pc-layout-2"><article class="pc-card pc-practice-board"><div class="pc-card-title"><div><span class="pc-eyebrow">练习流程</span><h2>{{ practiceScreenTitle }}</h2></div><span class="pc-pill">{{ practiceScenario.meta }}</span></div><template v-if="['practice','bridge','confirm'].includes(screenKey)"><div class="pc-test-tools"><button class="pc-secondary" @click="toggleSpeech(practiceActiveScreen.expression + '。' + practiceActiveScreen.hint)"><VolumeX v-if="isSpeaking" :size="16" /><Volume2 v-else :size="16" />{{ isSpeaking ? '停止朗读' : '朗读题目' }}</button><button class="pc-secondary" @click="downloadPractice"><Download :size="16" />下载试题</button></div><span class="pc-progress-label">{{ practiceActiveScreen.progress }}</span><div class="pc-question-board"><p>{{ practiceActiveScreen.hint }}</p><strong>{{ practiceActiveScreen.expression }}</strong></div><div class="pc-answer-grid"><button v-for="answer in practiceActiveScreen.answers" :key="answer.label" @click="handleAction(answer.correct ? 'ANSWER_CORRECT' : 'ANSWER_WRONG')">{{ answer.label }}</button></div></template><template v-else-if="screenKey === 'wrong'"><h3>{{ practiceFlow.wrong.title }}</h3><p>{{ practiceFlow.wrong.summary }}</p><div class="pc-tint-note">{{ practiceFlow.wrong.keyline }}</div><button class="pc-primary" @click="handleAction('CONTINUE')">{{ practiceFlow.wrong.buttonLabel }}</button></template><template v-else-if="['bridgeRetry','confirmRetry'].includes(screenKey)"><h3>{{ practiceRetry.cardTitle }}</h3><p>{{ practiceRetry.explanation }}</p><div class="pc-step-list"><div v-for="(step,index) in practiceRetry.steps" :key="step"><b>{{ index + 1 }}</b><span>{{ step }}</span></div></div><button class="pc-primary" @click="handleAction('TRY_AGAIN')">再试一次</button></template><template v-else><h3>{{ practiceFlow.mastered.title }}</h3><p>{{ practiceFlow.mastered.summary }}</p><div class="pc-metric-row"><div><strong>{{ practiceFlow.mastered.currentAccuracy }}%</strong><span>当前正确率</span></div><div><strong>{{ practiceFlow.mastered.closedKnowledgePoints }}</strong><span>已补会知识点</span></div></div><button class="pc-primary" @click="handleAction('GO_HOME')">回到首页</button></template></article><aside class="pc-card"><h3>本组练习</h3><div class="pc-selection-list"><div><span>预计用时</span><strong>{{ practiceScenario.estimatedMinutes }} 分钟</strong></div><div><span>题目数量</span><strong>{{ practiceScenario.totalQuestionCount }} 道</strong></div><div><span>练习重点</span><strong>{{ practiceScenario.focus }}</strong></div></div><div class="pc-preview-list"><div v-for="item in practiceScenario.previewQuestions" :key="item.prompt"><strong>{{ item.prompt }}</strong><small>{{ item.meta }}</small></div></div></aside></section></template>

      <template v-else-if="screenKey === 'photo' || screenKey === 'photoAnalyzing' || screenKey === 'photoResult'"><section class="pc-layout-2 pc-photo-layout"><article v-if="screenKey !== 'photoResult'" class="pc-card pc-upload-board"><span class="pc-eyebrow">作业检查</span><h2>把作业图片放到这里</h2><p>上传作业图片后，系统会识别题目、分析作答并生成解析。</p><div class="pc-photo-flow-steps"><span :class="{ active: screenKey === 'photo' }">1 上传作业</span><span :class="{ active: screenKey === 'photoAnalyzing' }">2 智能识别</span><span>3 查看结果</span></div><div class="pc-upload-visual"><div :class="['pc-dropzone', { 'pc-ai-scanning': screenKey === 'photoAnalyzing' }]"></div><template v-if="screenKey === 'photoAnalyzing'"><div class="pc-ai-orb" aria-hidden="true"><i /><i /><i /><span>AI</span></div><strong>正在识别题目与错误原因</strong><small class="pc-ai-caption">正在读取题目结构 · 检查作答步骤 · 生成解析</small></template><template v-else><div class="pc-upload-empty"><Camera :size="34" /><strong>拖拽图片或点击开始识别</strong><button class="pc-primary" @click="handleAction('ANALYZE_PHOTO')">开始识别</button></div></template></div></article><aside v-if="screenKey === 'photoResult'" class="pc-card pc-photo-result-panel"><div class="pc-photo-summary"><strong>发现 {{ photoCheckData.summary.focusCount }} 道需要重点讲解</strong><span>{{ photoCheckData.summary.correctCount }} 道计算正确，已生成对应解析</span></div><h3>逐题结果</h3><div class="pc-detail-list"><div v-for="question in photoCheckData.questions" :key="question.id" class="pc-detail-row"><div><strong>第 {{ question.id }} 题 · {{ question.expression }}</strong><small>孩子答案：{{ question.studentAnswer }} · 正确答案：{{ question.correctAnswer }} · {{ question.note }}</small></div><span>{{ question.statusLabel }}</span></div></div><section v-for="question in photoFocusQuestions" :key="`focus-${question.id}`" class="pc-photo-explanation"><div class="pc-card-title"><strong>{{ question.analysisTitle }}</strong><span class="pc-pill">{{ question.knowledgePoint }}</span></div><p>{{ question.diagnosis }}</p><ol><li v-for="step in question.steps" :key="step">{{ step }}</li></ol><div class="pc-tint-note"><strong>记住：</strong>{{ question.takeaway }}</div></section><div class="pc-photo-result-actions"><button class="pc-primary" @click="handleAction('PRACTICE_MISTAKE')">练重点错题 <ArrowRight :size="18" /></button><button class="pc-secondary" @click="handleAction('OPEN_PHOTO')">重新上传</button></div></aside><aside v-else class="pc-card"><h3>识别说明</h3><p>上传作业图片后，系统会完成识别、分析和解析。</p><div class="pc-step-list"><div><b>1</b><span>上传一张作业图片</span></div><div><b>2</b><span>系统识别题目和错误原因</span></div><div><b>3</b><span>查看解析并进入错题练习</span></div></div></aside></section></template>
      ...
      <template v-else-if="isAccountScreen"><section class="pc-layout-2"><article class="pc-card pc-main-card"><span class="pc-eyebrow">账号服务</span><h2>{{ screenMeta.title }}</h2><p class="pc-lead">{{ accountDescription }}</p><template v-if="screenKey === 'schoolBinding'"><div class="pc-status-banner" :class="{ active: bindingActive }"><strong>{{ bindingActive ? '家校绑定已确认' : '当前未绑定老师端' }}</strong><small>{{ bindingActive ? `${demoData.teacher.name} · 学情正在同步` : '填写绑定码后提交确认' }}</small></div><label class="pc-field"><span>老师绑定码</span><input v-model="bindingCode" placeholder="例如：WLS-301" /></label><button class="pc-primary" @click="toggleBinding">{{ bindingActive ? '取消当前绑定' : '提交绑定确认' }}</button><span v-if="bindingFeedback" class="pc-save-note">{{ bindingFeedback }}</span></template><template v-else-if="screenKey === 'learningReminder'"><div class="pc-switch-row"><div><strong>学习提醒</strong><small>{{ reminderEnabled ? '每天提醒一次' : '提醒已关闭' }}</small></div><button :class="['pc-switch', { active: reminderEnabled }]" @click="reminderEnabled = !reminderEnabled"><span>{{ reminderEnabled ? '开' : '关' }}</span></button></div><div class="pc-choice-tabs"><button v-for="time in reminderTimes" :key="time" :class="{ active: reminderTime === time }" @click="reminderTime = time">{{ time }}</button></div><div class="pc-tint-note">今天 {{ reminderTime }} 提醒：{{ demoData.student.name }} 先完成 {{ demoData.task.durationMinutes }} 分钟“{{ demoData.task.title }}”练习。</div><button class="pc-primary" @click="reminderSaved = true">{{ reminderSaved ? '提醒设置已保存' : '保存提醒设置' }}</button></template><template v-else-if="screenKey === 'dataPrivacy'"><div class="pc-form-grid"><label class="pc-field"><span>家长姓名</span><input v-model="guardianName" :readonly="!privacyEditing" /></label><label class="pc-field"><span>手机号</span><input v-model="guardianPhone" :readonly="!privacyEditing" /></label><label class="pc-field"><span>孩子姓名</span><input v-model="studentName" :readonly="!privacyEditing" /></label><label class="pc-field"><span>年级 / 班级</span><input v-model="className" :readonly="!privacyEditing" /></label></div><button class="pc-secondary" @click="togglePrivacyEditing">{{ privacyEditing ? '保存资料' : '修改资料' }}</button><span v-if="!privacyEditing && privacySaved" class="pc-save-note">资料已保存</span><div class="pc-tint-note">孩子姓名、班级和错题数据仅用于学习服务，家长与已绑定老师可见。</div></template><template v-else-if="screenKey === 'accountSettings'"><div class="pc-setting-group"><div class="pc-setting-head"><strong>系统字体</strong><small>当前：{{ fontSize }}</small></div><div class="pc-choice-tabs"><button v-for="size in fontSizes" :key="size" :class="{ active: fontSize === size }" @click="fontSize = size">{{ size }}</button></div></div><div class="pc-switch-row"><div><strong>声音提醒</strong><small>练习完成和提醒到达时播放提示音</small></div><button :class="['pc-switch', { active: soundEnabled }]" @click="soundEnabled = !soundEnabled"><span>{{ soundEnabled ? '开' : '关' }}</span></button></div><div class="pc-setting-group"><div class="pc-setting-head"><strong>账号操作</strong><small>谨慎处理</small></div><button class="pc-danger-link" @click="showCancelModal = true">注销账号 <ChevronRight :size="16" /></button></div></template><template v-else><div class="pc-danger-panel"><strong>注销后将不能继续查看孩子学习服务</strong><p>如果只是暂时不用，建议返回设置页选择“退出登录”。</p><label class="pc-check-row"><input v-model="cancelUnderstood" type="checkbox" /> 我已经知道注销会影响账号和学习数据查看</label></div><button v-if="!cancelConfirm" class="pc-primary pc-danger" :disabled="!cancelUnderstood" @click="cancelConfirm = true">进入二次确认</button><div v-else-if="!cancelSubmitted" class="pc-confirm-panel"><strong>再次确认注销账号</strong><p>如果确定不再使用这个账号，再提交注销申请。</p><button class="pc-secondary" @click="cancelConfirm = false">先不注销</button><button class="pc-danger" @click="cancelSubmitted = true">确认提交注销申请</button></div><span v-else class="pc-save-note">注销申请已提交。</span></template></article><aside class="pc-card pc-side-stack"><h3>{{ accountDetailTitle }}</h3><div class="pc-selection-list"><div v-for="item in accountDetailItems" :key="item.label"><span>{{ item.label }}</span><strong>{{ item.value }}</strong></div></div></aside></section></template>
...
import { afterClassReviewExam, demoData, featureOverviews, homeFeatures, photoCheckData, practiceEntryScenarios, practiceFlows, type PracticeEntryKey, type ScreenKey, type WrongBookTopicKey, wrongBookTopics } from "../data/demoData";
...
const accountItems = computed(() => [{ title: "家校绑定", summary: "老师与家长共享同一份学情", event: "OPEN_SCHOOL_BINDING" as DemoEvent, icon: "校" }, { title: "学习提醒", summary: "每天 1 条消息，提醒完成任务", event: "OPEN_LEARNING_REMINDER" as DemoEvent, icon: "提" }, { title: "数据与隐私", summary: "账号、孩子信息和数据范围", event: "OPEN_DATA_PRIVACY" as DemoEvent, icon: "隐" }, { title: "设置", summary: "字体、声音、密码和注销", event: "OPEN_ACCOUNT_SETTINGS" as DemoEvent, icon: "设" }, { title: loggedOut.value ? "已退出登录" : "退出登录", summary: loggedOut.value ? "当前已退出登录" : "退出当前家长账号", event: "OPEN_ACCOUNT_SETTINGS" as DemoEvent, icon: "出", action: "logout" as const }]);
